...
function isLoggedIn(req, res, next) {
  try {
    ...
    if (!token)
      return res
        .status(401)
        .json({ message: "Authentication is required to access this resource." , success: false });
    ...
    res.status(401).json({ message: "Authentication is required to access this resource.", success: false });
  }
}
